...
class StateStore:
    ...
    async def open(self) -> None:
        ...
        logger.info("StateStore opened: %s", self.db_path)
    ...
    async def persist_inventory(self, connection_id: str, snapshot_dict: dict[str, Any]) -> None:
        ...
        for dev in snapshot_dict.get("devices", []):
            await self.save_device(dev["device_id"], connection_id, dev)
        for ep in snapshot_dict.get("endpoints", []):
            await self.save_endpoint(ep["endpoint_id"], ep["device_id"], ep)
        for pt in snapshot_dict.get("points", []):
            await self.save_point(pt["point_id"], pt["endpoint_id"], pt)
